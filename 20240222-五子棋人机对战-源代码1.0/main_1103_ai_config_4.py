...
record_file_path=r"C:\Users\29098\Desktop\棋谱.xlsx"
# 创建或打开记录文件名字的xlsx文件
...
def value_all(board_inner, temp_list, value_model, chr,round_count):
    ...
    add = additional(temp_list,round_count)
    # print(temp_list)
...
def value_chess(board_inner):
    ...
    tp_list_o_2 = []
    tp_list_d = []
    score_x = value_all(board_inner, temp_list_x, value_model_X, 'X',1)  # 落子前，黑棋分数
    pos_x = (0, 0)
    score_o = value_all(board_inner, temp_list_o, value_model_O, 'O',1)  # 落子前，白棋分数
    pos_o = (0, 0)
    pos_d = (0, 0)
    score_x_2 = 0
    score_o_2 = 0
    ...
    for x in range(*range_x):
        for y in range(*range_y):
            ...
            if board_inner[x][y] != ' ':
                ...
                # 假定在该位置落黑子
                board_inner[x][y] = 'X'
                score_a = value_all(board_inner, tp_list_x, value_model_X, 'X',round_count)  # 该位置落黑子，黑棋分数
                score_c = value_all(board_inner, tp_list_c, value_model_O, 'O',round_count)  # 该位置落黑子，白棋分数
                ...
                # 假定在该位置落白子
                board_inner[x][y] = 'O'
                score_b = value_all(board_inner, tp_list_o, value_model_O, 'O',round_count)  # 该位置落白子，白棋分数
    ...
    if score_x_2 >= 1000:
        ...
        score_o_e = value_all(board_inner, temp_list_o, value_model_O, 'O',round_count)
        board_inner[x][y] = ' '
        print('执行策略1、直接获胜')
        ...
        board_inner[x][y] = 'X'
        temp_list_x.clear()
        score = value_all(board_inner, temp_list_x, value_model_X, 'X',1)
        score_o_e = value_all(board_inner, temp_list_o, value_model_O, 'O',1)
        board_inner[x][y] = ' '
        pos = pos_o
        ...
        temp_list_x.clear()
        temp_list_o.clear()
        score = value_all(board_inner, temp_list_x, value_model_X, 'X',round_count)
        score_o_e = value_all(board_inner, temp_list_o, value_model_O, 'O',round_count)
        board_inner[x][y] = 'O'
        score_test = value_all(board_inner, [], value_model_O, 'O',round_count)
        board_inner[x][y] = ' '
        pos = pos_d
        print('黑棋原得分', score_x)
        print('黑棋得分', score)
...
def main(board_inner):
    ...
    box=pg.image.load("data/ceshinidema.png").convert_alpha()
